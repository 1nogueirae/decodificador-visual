...
const translateButton = document.querySelector('#translateButton') as HTMLButtonElement;
...
function mountTable(id: string, lat: string, lng: string, ignState: string) {
...
function validateLogInput(logInput: string): boolean {
    if (logInput !== '' && logInput.startsWith('START;') && logInput.endsWith(';END')) {
        return true;
    } return false;
}

function parseLogInput(logInput: string): string[] {
...
logTextArea?.addEventListener('input', () => {
    ...
    if (logTextArea && logTextArea instanceof HTMLTextAreaElement) {
...
resultTable?.addEventListener('click', (event) => {
    if (event.target && (event.target as HTMLElement).id === 'clearButton') {
